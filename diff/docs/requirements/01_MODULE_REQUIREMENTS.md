...
| 模块                  | 前置依赖                                 | 主要用户价值                     |
| --------------------- | ---------------------------------------- | -------------------------------- |
| 工程与决策基线        | 无                                       | 可重复开发、测试、部署和观测     |
| 身份、角色与知识空间  | 工程与决策基线                           | 安全访问和知识治理边界           |
| 文档接入与任务中心    | 身份、角色与知识空间                     | 文档可上传、追踪和恢复           |
| 文件安全、解析与 OCR  | 文档接入与任务中心                       | 异构文件变成统一可定位 Block     |
| 知识加工、质量与审核  | 文件安全、解析与 OCR                     | Block 变成受控、可审核的 Chunk   |
| 向量化、索引与发布    | 知识加工、质量与审核                     | 知识安全进入或退出在线检索       |
| 会话、Run 与事件底座  | 身份、角色与知识空间                     | 问答请求可追踪、取消和续传       |
| Query Plan 与混合检索 | 向量化、索引与发布；会话、Run 与事件底座 | 从正确版本和权限范围找到证据     |
| 证据、生成与答案校验  | Query Plan 与混合检索                    | 生成有依据、可核验、会拒答的答案 |
| 评测与生产可靠性      | 前述所有后端能力                         | 质量、性能和故障恢复达到上线要求 |
| Web 产品与全链路验收  | 管理端、问答端及评测与生产可靠性         | 管理和问答用户具备完整使用闭环   |
...
需求到源码、自动化测试和浏览器验收的映射见 [身份权限与知识空间 实施与验收证据](./identity-access-implementation-evidence.md)。
...
需求到源码、自动化测试、故障注入和浏览器验收的映射见 [文档接入与任务 实施与验收证据](./document-ingestion-implementation-evidence.md)。真实 MinIO 镜像因本机 Docker Registry 超时未完成环境联调，已在证据与 Runbook 中明确保留补跑项；代码、契约和其余门禁不以 Fake 结果冒充该项环境证据。
...
- [x] `PAR-002` 集成可配置内容安全扫描 Port；默认内置流式规则拒绝 EICAR、可执行魔数和超限输入，并明确不冒充完整病毒特征库。
- [x] `PAR-003` 检查宏、嵌入对象、外部链接、密码保护、压缩层数、解压比例、页数、像素和表格规模限制。
- [x] `PAR-004` Parser Runtime 使用无外网、只读根文件系统、最小权限和资源/时间上限的隔离容器。
- [x] `PAR-005` 定义 ParserPort 与目标级 OcrPort，输入输出使用 Zod 校验并携带引擎版本。
- [x] `PAR-006` 建立 PDF、DOCX、XLSX、PPTX、图片、HTML、Markdown、TXT、CSV 的真实 Node 解析实现与格式路由。
- [x] `PAR-007` PDF 按页判断文字覆盖度，可靠文字层不做 OCR；扫描页及 Office 归档内嵌图片形成可定位 OCR Target。HTML/Markdown 外部图片默认不抓取。
...
- [x] `PAR-014` 为每类格式建立公开合成的真实 Golden 文档和 Block Snapshot，并覆盖 Office 安全样本。
- [x] `PAR-015` 支持管理员查看 Parser/OCR Profile、版本、耗时和失败原因。
- [x] `PAR-016` OCR 必须按目标校验完整性、唯一性和定位；只有实际返回且通过质量检查的 PAGE 结果可以替换原生页，空结果、缺失、重复、额外目标、低置信度和失败必须保留原生内容并形成可见 Issue 或质量门禁。
- [x] `PAR-017` 九类 Parser 共用可累计的解析资源预算，在循环、跨度展开和大对象分配前检查表格行列/实际与展开单元格、跨度、像素、输出规模和取消；超限使用稳定错误码，不静默截断。
- [x] `PAR-018` HTML 与 Markdown 按 DOM 阅读顺序恢复裸文本、换行、代码、引用、层级列表、图片说明和表格；正文不得遗漏或重复，合并范围必须落在输出矩阵内，`originalText` 不得被后续标准化覆盖。
- [x] `PAR-019` XLSX 图片通过真实媒体/relationship 关联到 Sheet 与锚点；公式、缓存结果、显示格式、源行列和合并范围可回溯，不按图片文件名字典序猜测。
- [x] `PAR-020` PPTX 通过 `presentation.xml` 与 relationships 恢复真实页序，正确拼接段落 Run、解析表格合并并统计原生表格文本；未支持的备注、图表、SmartArt、隐藏页和复杂坐标必须形成明确告警。
- [x] `PAR-021` DOCX 图片通过 drawing relationship 区分媒体资产与出现位置，并把 OCR 内容关联到正文锚点；不伪造物理页码，未知尺寸、页眉页脚、脚注、文本框和修订内容的支持边界必须可见。
- [x] `PAR-022` PDF 使用可离线部署的 Node 结构抽取恢复文字项、行段、阅读顺序和标题层级；标题推断记录依据、置信度与算法 revision，并覆盖页眉页脚、分栏、旋转、表格去重、扫描/混合页和异常文字映射。
- [x] `PAR-023` TXT、CSV 和图片明确编码、BOM、换行、表头、分隔符、多行字段、不规则列、多帧、方向和损坏输入边界；不支持能力必须拒绝或告警，不得静默少解析。
- [x] `PAR-024` Parser 语义变更必须升级算法 revision，用新 content revision 重处理并通过质量审核、索引对账后原子发布；旧快照不得跨 revision 复用，回滚不得覆盖当前已发布事实。
...
需求到源码、自动化测试、真实 PostgreSQL 并发/重处理验证和环境补跑项见 [知识加工与质量 实施与验收证据](./knowledge-processing-implementation-evidence.md)。外网使用真实 `cl100k_base` BPE 完成工程门禁；内网 Embedding 官方 tokenizer 与业务 Golden 仍须按验收清单复验，不能将当前 Profile 冒充模型精确计数。
...
需求到源码、自动化测试、真实 PostgreSQL 场景、可观测性和内网补跑项的完整映射，见 [索引构建与发布 实施与验收证据](./indexing-publication-implementation-evidence.md)。企业 Milvus、Embedding 服务和脱敏业务集的环境验收仍需在内网执行，未使用外网 Fixture 冒充该项证据。
...
需求到源码、自动化、真实 PostgreSQL/Redis、运行时烟测和内网补跑项的映射见 [会话运行与事件 实施证据](./conversation-runtime-implementation-evidence.md) 与 [会话运行与事件 验收清单](../acceptance/conversation-runtime-acceptance.md)。会话运行与事件 完成的是可持久化运行容器；真实检索和模型生成仍按基线由 查询规划与混合检索/证据与答案生成 实现，不以 Fixture 答案冒充。
...
- [x] `RET-008` Query Embedding 使用锁定 Profile；缓存 Key 只由真实输入文本 Hash、模型身份、revision、维度、归一化/输出模式和查询模板版本组成，不包含 Run 时间、完整计划或权限范围。缓存只复用向量，命中后仍执行当前权限、发布版本和生效期复核。
- [x] `RET-009` Dense 与 Sparse 并行检索，多子问题、多空间的远程检索调用受 Run 快照并发上限约束；单路失败时按策略安全降级并记录标志。
...
- [x] `RET-014` 初始 topK、Reranker 候选池 topK、最终 topK、检索并发上限、RRF K、Dense/Sparse 权重按知识空间 Profile 配置并进入 Run 快照；最终 topK 只能在 Reranker 后裁剪。
...
- [x] `ANS-002` 使用专用 Reranker 对受控候选池精排，精排后再按最终 topK 裁剪，支持超时和受控降级。
...
- [x] `ANS-007` ContextBuilder 进行 Token Budget、覆盖驱动的动态单文档配额、重叠窗口压缩、来源多样性和明确边界标记；保留标题路径、版本与适用期，来源中的指令只视为数据。
...
- [x] `ANS-014` 可修复 Draft 最多重生成一次；剩余 Deadline 不足时不启动修复生成，超过上限或预算不足转部分回答或拒答。
...
- 全部语义模块与 NFR 需求已完成，或已通过正式变更流程移出范围。
