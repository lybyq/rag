...
LLM_REWRITE_REQUEST_TIMEOUT_MS=10000
LLM_GENERATION_REQUEST_TIMEOUT_MS=20000
LLM_RERANK_REQUEST_TIMEOUT_MS=10000
LLM_JUDGE_REQUEST_TIMEOUT_MS=10000
...
| `RETRIEVAL_CANDIDATE_POOL_TOP_K`    | `40`               | `1..100`；介于 final 和 initial 之间        | 否   | 否     | 降回已评测候选池        |
| `RETRIEVAL_FINAL_TOP_K`             | `12`               | `1..50`；只在 Reranker 后裁剪               | 否   | 否     | 恢复黄金集基线          |
| `RETRIEVAL_MAX_CONCURRENCY`         | `8`                | `1..64`；单 Run 的 Milvus 远程调用上限      | 否   | 否     | 降低以保护内网服务      |
...
| `RETRIEVAL_QUERY_CACHE_TTL_SECONDS` | `600`              | `10..86400`；Key 绑定真实输入和模型配置     | 否   | 否     | 降低 TTL 或清空命名空间 |
...
| 环境变量                               | 默认值  | 作用与约束                                                       |
| -------------------------------------- | ------- | ---------------------------------------------------------------- |
| `LLM_REQUEST_TIMEOUT_MS`               | `60000` | Provider 总的单次上限；下面每个任务预算都不能大于它              |
| `LLM_REWRITE_REQUEST_TIMEOUT_MS`       | `10000` | 可选问题改写；预算不足或失败时使用确定性原问题计划               |
| `LLM_GENERATION_REQUEST_TIMEOUT_MS`    | `60000` | 必需的结构化答案生成；仍受 Run 总 Deadline 限制                  |
| `LLM_RERANK_REQUEST_TIMEOUT_MS`        | `10000` | 可选 LLM 证据排序；不是专用 BGE Reranker 的超时                  |
| `LLM_JUDGE_REQUEST_TIMEOUT_MS`         | `10000` | 只检查确定性规则无法判断的语义 Claim                             |
| `ANSWER_REGENERATION_MIN_REMAINING_MS` | `15000` | 低于此剩余时间不启动第二份草稿，转为安全拒答而不是发布未校验内容 |

这些值在应用启动时读取，修改后需要重启相应服务。`ANSWER_LLM_EVIDENCE_RERANK_ENABLED` 和
`ANSWER_SEMANTIC_JUDGE_ENABLED` 的 env 值只负责数据库首次初始化默认值；已有数据库中的系统/空间
Feature Flag 才是新 Run 的实际决定，进行中的 Run 使用创建时冻结的决定。
