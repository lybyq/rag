...
## 8. 为什么答案 Worker 改成“空一位补一位”

旧调度器一次领取十条，再 `Promise.all` 等十条全部结束。假设九条各 3 秒、一条 60 秒，九个
执行位从第 3 秒一直空到第 60 秒，新问题只能等下一批。现在 `inFlight` 集合代表固定槽位：
领取数量等于“配置并发数减当前集合大小”；某个 Promise 完成后先从集合删除，再触发一次领取。
数据库 `FOR UPDATE SKIP LOCKED` 租约仍负责多副本互斥，所以这不是在内存里假装去重。

关闭时先设置 `stopping`，不再补位；若数据库领取正在进行则等领取结束，再等待已经领取的 Run。
这避免容器退出时刚拿到租约却立刻丢掉任务。排障看领取批次是否始终等于空闲槽数，不应看到
一个慢 Run 把其他槽位长期闲置。

## 9. 为什么 Embedding 缓存键不放 userId 和 Plan Hash

Embedding 的数学输入是“查询文本 + 模型/revision + 查询模板 + 维度 + 归一化 + 输出模式”。同一句
问题换一个 Run 时间、用户或可见空间，向量不会变化。旧 Key 把包含 `asOf` 的 Plan Hash 放进去，
导致每次都重新算向量。新 Key 只保存上述会改变数值的事实，并先 Hash 问题正文，Redis Key 不泄漏
原文。它没有缓存 Milvus 命中或 PG 正文，所以命中后仍按本次用户、空间、Manifest、版本和生效期
重新检索与回源；“向量可共享”不等于“权限结果可共享”。

## 10. 三层 TopK 和有界并发怎么读

`initialTopK` 是每个子问题、每个空间、每条 Dense/Sparse 路线向 Milvus 要多少；RRF 合并、PG
回源和来源多样性以后保留 `candidatePoolTopK` 给 BGE；BGE 精排完才按 `finalTopK` 收敛。以前在
BGE 前就裁到 final，会让语义相关但向量初排稍后的正确条款永远没有翻盘机会。

请求数量大约是“子问题数 × 可见空间数 × 可用路线数”。图用固定 worker 消费这个任务表，整体
同时在途不超过 `maxConcurrency`，每次调用仍携带同一个 Deadline 和 AbortSignal。某一路失败后，
该路线尚未启动的任务直接跳过，另一条路线继续形成安全降级。第二轮问题归一化后若首轮已经执行，
记录 `RETRY_QUERY_UNCHANGED` 并结束，不再重复花一次 Embedding 与 Milvus 时间。

## 11. 分阶段预算为什么不是简单把总超时调小

长答案生成可能需要 30～60 秒，但 Rewrite、证据排序和 Judge 通常只应占几秒。如果全部使用同一个
60 秒超时，三个可选调用任何一个卡住都会拖长 P95。现在四类调用各有预算，总 Run Deadline 仍是
最外层硬上限。Rewrite 或 LLM 证据排序预算不足可以安全跳过；Judge 预算不足不能当成“支持”；
需要修复的草稿若剩余时间不足，Validator 明确写入 `REGENERATION_BUDGET_EXHAUSTED` 并拒答。

设置页显示的是数据库实际 Flag。env 只在首次初始化数据库时给默认值，后来改 env 不会覆盖已经
存在的记录；新 Run 冻结当时决定，在途 Run 不变。这是排查“我明明关了变量为什么还调用模型”时
最先要确认的三层事实。

## 12. Context 去重与动态单文档配额

Evidence 扩展常把命中 Child、Parent 和相邻 Chunk 一起拿回，Chunk 自身又有 overlap。Context
先为每个子问题选一个代表证据，这些来源永不因去重被删；再对辅助 Parent/Neighbor 去掉已经逐字
出现的窗口，并过滤高度重叠句子，只保留新增条件。`maximumPerDocument` 仍约束普通补充材料，但
如果一份制度确实分别回答三个子问题，会动态提高到三个代表来源，避免固定“每文档两条”截断关键
条款。总 Token Budget 始终是硬上限。

每条模型证据同时携带标题路径、content revision 和生效起止日期。生成 Prompt 要求先直接回答，
但条件、例外、版本和冲突不能为了短而删除。最终生成、确定性计算、语义 Judge 与 Validator 都只
读取 Context 实际保留的窗口，不会拿模型没看见的完整正文替它补证据。
