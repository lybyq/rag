...
export interface DocumentParserLimits {
  ...
  /** 合并单元格按跨度展开后的累计占位上限；省略时等于 maxTableCells。 */
  readonly maxExpandedTableCells?: number;
  /** 单张表最大输出行数；省略时由 maxTableCells 推导。 */
  readonly maxTableRows?: number;
  /** 单张表最大输出列数；省略时由 maxTableCells 推导。 */
  readonly maxTableColumns?: number;
  /** 单个 rowspan/colspan 的最大值；省略时使用有界兼容值。 */
  readonly maxTableSpan?: number;
  /** 全文 Block 原文字符总量；省略时由 maxInputBytes 安全推导。 */
...
export function emptyInspection(): FileStructureInspection {
  return {
    ...
    totalPixels: null,
    tableCellCount: null,
    expandedTableCellCount: null,
    maximumTableRowCount: null,
    maximumTableColumnCount: null,
    maximumTableSpan: null,
