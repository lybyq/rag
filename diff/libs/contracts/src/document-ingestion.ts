 ...
 * 外部业务文件描述。
 *
 * externalSourceId + externalDocumentId 是长期业务主键，sha256 是内容身份；三者不能用
 * clientFileId 替代。clientFileId 只负责一次批次内把浏览器/调用方文件与服务端计划对应起来。
 */
export const DocumentBatchFileDescriptorSchema = UploadFileDescriptorSchema.extend({
  externalSourceId: z.string().trim().min(1).max(128),
  externalDocumentId: z.string().trim().min(1).max(256),
  sha256: Sha256Schema,
});
/** 外部业务文件描述类型。 */
export type DocumentBatchFileDescriptor = z.infer<typeof DocumentBatchFileDescriptorSchema>;

/** 外部业务批次请求；幂等键从 HTTP Header 读取，Body 只保存可哈希的文件计划。 */
export const CreateDocumentBatchRequestSchema = z.object({
  files: z.array(DocumentBatchFileDescriptorSchema).min(1).max(100),
});
/** 外部业务批次请求类型。 */
export type CreateDocumentBatchRequest = z.infer<typeof CreateDocumentBatchRequestSchema>;

/** 外部业务批量直传创建结果；batchId 复用上传会话 ID，避免产生第二套生命周期事实。 */
export const CreateDocumentBatchResultSchema = z.object({
  batchId: z.uuid(),
  uploadSession: UploadSessionSchema,
  replayed: z.boolean(),
});
/** 外部业务批量直传创建结果类型。 */
export type CreateDocumentBatchResult = z.infer<typeof CreateDocumentBatchResultSchema>;

/** 批次内单文件的上传与处理状态；未完成上传时 jobId 必须保持 null。 */
export const DocumentBatchFileStatusSchema = z.object({
  fileId: z.uuid(),
  clientFileId: z.string().min(1).max(100),
  externalSourceId: z.string().min(1).max(128),
  externalDocumentId: z.string().min(1).max(256),
  originalFileName: z.string().min(1).max(240),
  uploadStatus: z.enum(['PENDING', 'COMPLETED', 'CANCELLED']),
  documentId: z.uuid().nullable(),
  documentVersionId: z.uuid().nullable(),
  jobId: z.string().min(1).max(300).nullable(),
  jobStatus: IngestionExecutionStatusSchema.nullable(),
  currentStep: IngestionStepNameSchema.nullable(),
  overallPercent: z.number().min(0).max(100).nullable(),
  publicMessage: z.string().max(500).nullable(),
});
/** 批次内单文件状态类型。 */
export type DocumentBatchFileStatus = z.infer<typeof DocumentBatchFileStatusSchema>;

/** 外部业务轮询批次状态；每个文件独立成功或失败，不做批次级回滚。 */
export const DocumentBatchStatusSchema = z.object({
  batchId: z.uuid(),
  spaceId: z.uuid(),
  status: UploadSessionStatusSchema,
  files: z.array(DocumentBatchFileStatusSchema).max(100),
});
/** 外部业务批次状态类型。 */
export type DocumentBatchStatus = z.infer<typeof DocumentBatchStatusSchema>;
...
export const CreateDocumentBatchEnvelopeSchema = createApiEnvelopeSchema(
  CreateDocumentBatchResultSchema,
);
export const DocumentBatchStatusEnvelopeSchema = createApiEnvelopeSchema(DocumentBatchStatusSchema);
