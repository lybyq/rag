...
export const RerankScoreSchema = z.object({
  candidateId: z.string().min(1).max(180),
  // Provider 的 logit 必须在 Adapter 内只归一化一次；跨层契约永远是可比较的 0..1 概率。
...
export const EvidenceSourceSchema = z.object({
  ...
  /** 可审计的覆盖依据；记录词面比例或专用 Reranker 门槛，不能只因“有候选”就算覆盖。 */
  coverageBasis: z
    .array(
      z.object({
        subQuestionIndex: z.number().int().nonnegative(),
        lexicalOverlapRatio: z.number().min(0).max(1),
        acceptedBy: z.enum(['LEXICAL', 'RERANKER']),
      }),
    )
    .max(4)
    .optional(),
...
export const AnswerContextSchema = z.object({
  ...
  /** 实际进入模型的来源窗口；截断后的正文必须与 prompt 完全一致，供 Validator 复核。 */
  sourceWindows: z
    .array(
      z.object({
        sourceId: z.uuid(),
