...
export const RagRunSnapshotSchema = z.object({
  ...
  /** 前端显示“等待较久”提示的服务端阈值；仅表示耗时，不伪造业务进度。 */
  slowNoticeSeconds: z.number().int().min(1).max(3_600).optional(),
...
/** 用户可见处理说明阶段；它描述可观察事实，不包含模型私有思维链。 */
export const RagRunProgressPhaseSchema = z.enum([
  'RETRIEVAL',
  'FILTERING',
  'GENERATION',
  'VALIDATION',
...
/** 用户可见处理说明阶段类型。 */
export type RagRunProgressPhase = z.infer<typeof RagRunProgressPhaseSchema>;

/** `run.progress` 的白名单 Payload；计数均来自授权后的实际节点结果。 */
export const RagRunProgressPayloadSchema = z.object({
  phase: RagRunProgressPhaseSchema,
  status: z.enum(['RUNNING', 'COMPLETED', 'FAILED']),
  publicMessage: z.string().min(1).max(500),
  attempt: z.number().int().positive(),
  authorizedSpaceIds: z.array(z.uuid()).max(50).optional(),
  accessibleDocumentCount: z.number().int().nonnegative().optional(),
  includedEvidenceCount: z.number().int().nonnegative().optional(),
  durationMs: z.number().int().nonnegative().optional(),
});
/** 可重放的真实处理说明 Payload。 */
export type RagRunProgressPayload = z.infer<typeof RagRunProgressPayloadSchema>;
