...
export const RetrievalProfileSnapshotSchema = z
  .object({
    ...
    /** 升级前 Run 可缺省；新 Run 必须由配置写入，Reranker 在这个池上判断。 */
    candidatePoolTopK: z.number().int().min(1).max(100).optional(),
    finalTopK: z.number().int().min(1).max(50),
    /** 升级前 Run 可缺省；图执行时使用安全默认值兼容历史记录。 */
  ...
  .superRefine((profile, context) => {
    ...
    if (profile.candidatePoolTopK !== undefined && profile.finalTopK > profile.candidatePoolTopK) {
      context.addIssue({
        code: 'custom',
        path: ['candidatePoolTopK'],
        message: 'Reranker 候选池不能小于最终 TopK',
      });
    }
    if (
      profile.candidatePoolTopK !== undefined &&
      profile.candidatePoolTopK > profile.initialTopK
    ) {
      context.addIssue({
        code: 'custom',
        path: ['candidatePoolTopK'],
