...
import {
  ...
  CreateDocumentBatchEnvelopeSchema,
  ...
  DocumentBatchStatusEnvelopeSchema,
...
export function buildBaseOpenApiDocument(options: OpenApiDocumentOptions): OpenApiDocument {
  ...
  const documentIngestionPaths: Record<string, unknown> = options.includeDocumentIngestion
    ? {
        ...
        '/api/v1/spaces/{spaceId}/document-batches': {
          post: {
            operationId: 'createDocumentBatch',
            summary: '为外部业务创建多文件直传批次',
            parameters: [
              spaceIdParameter,
              {
            ...
            requestBody: jsonBody('CreateDocumentBatchRequest'),
            responses: {
              '201': jsonResponse('批次 ID 与逐文件直传计划', 'CreateDocumentBatchEnvelope'),
              '413': errorResponse('文件数量或大小超过配置上限'),
              ...securedResponses,
            },
          },
        },
        '/api/v1/document-batches/{batchId}': {
          get: {
            operationId: 'getDocumentBatch',
            summary: '查询批次内逐文件上传与处理状态',
            parameters: [uuidPathParameter('batchId')],
            responses: {
              '200': jsonResponse('批次逐文件状态', 'DocumentBatchStatusEnvelope'),
              ...securedResponses,
            },
          },
        },
      ...
      schemas: {
        ...
        ...(options.includeDocumentIngestion
          ? {
              ...
              CreateDocumentBatchRequest: z.toJSONSchema(CreateDocumentBatchRequestSchema),
              ...
              CreateDocumentBatchEnvelope: z.toJSONSchema(CreateDocumentBatchEnvelopeSchema),
              DocumentBatchStatusEnvelope: z.toJSONSchema(DocumentBatchStatusEnvelopeSchema),
