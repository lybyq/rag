...
export const FileStructureInspectionSchema = z.object({
  ...
  /** 兼容新增的预算事实；历史 v2 响应可以没有这些字段。 */
  expandedTableCellCount: z.number().int().nonnegative().nullable().optional(),
  maximumTableRowCount: z.number().int().nonnegative().nullable().optional(),
  maximumTableColumnCount: z.number().int().nonnegative().nullable().optional(),
  maximumTableSpan: z.number().int().positive().nullable().optional(),
  outputCharacterCount: z.number().int().nonnegative().nullable().optional(),
  /** PDF Info/XMP 标题只作为文档元数据保存，不自动冒充正文标题。 */
  documentTitle: z.string().max(500).nullable().optional(),
  /** PDF 大纲节点数量；没有大纲时为 0，其他格式可以省略。 */
  outlineItemCount: z.number().int().nonnegative().nullable().optional(),
  /** 是否通过 PDF 对象模型完成附件检查；false 不能解释为“确认没有附件”。 */
  embeddedObjectInspectionComplete: z.boolean().optional(),
  /** PDF 文档级/页面级 JavaScript 或打开动作数量。 */
  activeActionCount: z.number().int().nonnegative().nullable().optional(),
});
