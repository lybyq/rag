...
export class PostgresDocumentProcessingRepository implements DocumentProcessingRepository {
  ...
  public async complete(command: CompleteDocumentProcessingCommand): Promise<void> {
    ...
    try {
      ...
      const processingMetrics = JSON.stringify({
        ...
        ocrOutcomeCounts: command.ocrOutcomeCounts,
      });
      if (command.requiresManualReview) {
        const reviewReason = command.reviewReason ?? '关键页面 OCR 结果不可用，等待人工审核';
        await client.query(
          `UPDATE document_parse_runs
              SET status = 'WAITING', page_count = $2, block_count = $3, ocr_page_count = $4,
                  derived_bucket = $5, derived_object_key = $6, derived_sha256 = $7,
                  metrics = metrics || $8::jsonb, failure_message = $9,
          ...
          [
            ...
            successfulOcrPageCount(command.blocks),
        ...
        await client.query(
          `UPDATE ingestion_job_steps
              SET status = 'WAITING', processed_units = GREATEST(processed_units, 1),
                  total_units = COALESCE(total_units, 1), public_message = $2,
                  finished_at = now(), updated_at = now()
            WHERE job_id = $1 AND step_name = 'NORMALIZE'`,
          [command.jobId, reviewReason],
        );
        await client.query(
          `UPDATE ingestion_jobs
              SET status = 'WAITING', current_step = 'NORMALIZE', overall_percent = 50,
                  public_message = $3, lease_owner = NULL, lease_expires_at = NULL,
                  heartbeat_at = now(), updated_at = now()
            WHERE id = $1 AND lease_owner = $2`,
          [command.jobId, command.workerId, reviewReason],
        );
        await client.query(
        ...
        await this.insertEvent(
          client,
          command.jobId,
          'ingestion.document_parsing_review_required',
          {
      ...
      await this.insertEvent(client, command.jobId, 'ingestion.document_parsing_completed', {
        ...
        ocrPageCount: successfulOcrPageCount(command.blocks),
        snapshotReused: command.snapshotReused,
      });
...
/**
 * 只统计最终真正进入统一 Block 的 OCR 页。
 * 缺失、空结果和低置信度即使 Provider 返回过 pageNo，也不能算作成功 OCR。
 *
 * @requirement PAR-016
 */
function successfulOcrPageCount(blocks: CompleteDocumentProcessingCommand['blocks']): number {
  return new Set(
    blocks
      .filter((block) => block.ocrEngine !== null && block.pageNo !== null)
      .map((block) => block.pageNo as number),
  ).size;
}
