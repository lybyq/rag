...
import {
  ApplicationError,
  progressWhenStepFinishes,
  progressWhenStepStarts,
  type AccessContext,
...
export class PostgresRagRunRepository implements RagRunRepository {
  ...
  public async startStep(runId: string, command: StartRagRunStepCommand): Promise<RagRunStep> {
    ...
    try {
      ...
      const publicProgress = progressWhenStepStarts(command.nodeKey, command.attempt);
      if (publicProgress) await this.appendEvent(client, runId, 'run.progress', publicProgress);
      await client.query('COMMIT');
      return mapStep(step);
    } catch (error) {
      await client.query('ROLLBACK');
  ...
  public async finishStep(runId: string, command: FinishRagRunStepCommand): Promise<RagRunStep> {
    ...
    try {
      ...
      const publicProgress = progressWhenStepFinishes(
        command.nodeKey,
        command.attempt,
        command.status,
