...
/** OPT-013：外部稳定文档身份指向最近一次已登记内容事实。 */
interface ExternalDocumentBindingRow extends QueryResultRow {
  document_id: string;
  last_content_sha256: string;
  latest_document_version_id: string;
  latest_document_file_id: string;
  latest_ingestion_job_id: string;
}
...
function mapUploadFile(row: UploadRow): UploadFileRecord {
  return {
    ...
    ...(row.external_source_id ? { externalSourceId: row.external_source_id } : {}),
    ...(row.external_document_id ? { externalDocumentId: row.external_document_id } : {}),
...
const uploadSelect = `
  SELECT us.id AS session_id, us.space_id, us.status AS session_status,
         ...
         uf.external_source_id, uf.external_document_id,
...
export class PostgresDocumentIngestionRepository implements DocumentIngestionRepository {
  ...
  public async createUploadSession(
    ...
    try {
      ...
      if (command.externalBatch) {
        await client.query(
          `INSERT INTO external_import_batches (
             space_id, idempotency_key, request_sha256, upload_session_id, created_by
           ) VALUES ($1, $2, $3, $4, $5)`,
          [
            command.spaceId,
            command.externalBatch.idempotencyKey,
            command.externalBatch.requestSha256,
            command.id,
            context.user.userId,
          ],
        );
      ...
      for (const file of command.files) {
        await client.query(
          `INSERT INTO upload_files (
             ...
             external_source_id, external_document_id
          ...
          [
            ...
            file.partSizeBytes,
            file.partCount,
            file.externalSourceId ?? null,
            file.externalDocumentId ?? null,
          ],
        );
      }
  ...
  /** OPT-013：只向创建者或系统管理员暴露幂等绑定，空间权限还会在应用层再次校验。 */
  public async findDocumentBatchByIdempotency(
    context: AccessContext,
    spaceId: string,
    idempotencyKey: string,
  ): Promise<{ uploadSessionId: string; requestSha256: string } | undefined> {
    const result = await this.pool.query<{
      upload_session_id: string;
      request_sha256: string;
    }>(
      `SELECT eib.upload_session_id, eib.request_sha256
         FROM external_import_batches eib
        WHERE eib.space_id = $1 AND eib.idempotency_key = $2
          AND ($3::boolean OR eib.created_by = $4)`,
      [spaceId, idempotencyKey, this.isSystemAdmin(context), context.user.userId],
    );
    const row = result.rows[0];
    return row
      ? { uploadSessionId: row.upload_session_id, requestSha256: row.request_sha256 }
      : undefined;
  }
  ...
  public async completeUpload(
    ...
    try {
      ...
      const externalIdentity =
        upload.external_source_id && upload.external_document_id
          ? {
              sourceId: upload.external_source_id,
              documentId: upload.external_document_id,
            }
          : undefined;
      const contentSha256 =
        command.object.sha256 ?? command.sha256 ?? upload.expected_sha256 ?? undefined;
      let externalBinding: ExternalDocumentBindingRow | undefined;
      if (externalIdentity) {
        if (!contentSha256) {
          throw new ApplicationError('OBJECT_MISMATCH', 409, '外部文档必须提供内容 SHA-256');
        }
        // 绑定行尚不存在时 FOR UPDATE 锁不到任何内容，因此先使用事务级 advisory lock。
        // 相同空间+外部主键的并发首次导入会串行，避免短暂创建两个平台 Document 再靠唯一约束回滚。
        await client.query('SELECT pg_advisory_xact_lock(hashtextextended($1, 0))', [
          `${upload.space_id}\u0000${externalIdentity.sourceId}\u0000${externalIdentity.documentId}`,
        ]);
        const bindingRows = await client.query<ExternalDocumentBindingRow>(
          `SELECT document_id, last_content_sha256, latest_document_version_id,
                  latest_document_file_id, latest_ingestion_job_id
             FROM external_document_bindings
            WHERE space_id = $1 AND external_source_id = $2 AND external_document_id = $3
            FOR UPDATE`,
          [upload.space_id, externalIdentity.sourceId, externalIdentity.documentId],
        );
        externalBinding = bindingRows.rows[0];
        if (externalBinding?.last_content_sha256 === contentSha256) {
          // 不同批次再次上传相同外部内容时，直接把本次 uploadFile 关联到原事实；既不创建新版本，
          // 也不产生第二个 Job/Outbox。应用层会根据返回 file.objectKey 与本次 objectKey 不同，
          // 对本次重复隔离对象执行尽力清理；清理失败不能回滚已经提交的幂等事实。
          await client.query(
            `UPDATE upload_files
            ...
            [
              command.uploadFile.id,
              externalBinding.document_id,
              externalBinding.latest_document_version_id,
              externalBinding.latest_document_file_id,
              externalBinding.latest_ingestion_job_id,
            ],
          );
          await this.completeSessionWhenNoPendingFile(client, upload.session_id);
          const replayed = await this.loadCompleteResult(client, command.uploadFile.id);
          await client.query('COMMIT');
          return replayed;
        }
      }

      const documentId = externalBinding?.document_id ?? randomUUID();
      ...
      const documentResult = externalBinding
      ...
      if (!documentRow) {
        throw new ApplicationError('INVALID_STATE', 409, '外部文档已归档，不能继续同步新版本');
      }
      const versionResult = await client.query<VersionRow>(
        ...
        [
          versionId,
          documentId,
          documentRow.latest_version_number,
          contentRevision,
          context.user.userId,
        ],
      );
      ...
      await this.completeSessionWhenNoPendingFile(client, upload.session_id);
      if (externalIdentity && contentSha256) {
        await client.query(
          `INSERT INTO external_document_bindings (
             space_id, external_source_id, external_document_id, document_id,
             last_content_sha256, latest_document_version_id, latest_document_file_id,
             latest_ingestion_job_id, created_by
           ) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
           ON CONFLICT (space_id, external_source_id, external_document_id)
           DO UPDATE SET last_content_sha256 = EXCLUDED.last_content_sha256,
                         latest_document_version_id = EXCLUDED.latest_document_version_id,
                         latest_document_file_id = EXCLUDED.latest_document_file_id,
                         latest_ingestion_job_id = EXCLUDED.latest_ingestion_job_id,
                         updated_at = now()`,
          [
            upload.space_id,
  ...
  /** 同一会话的 complete 事务持有 session 行锁；最后一个文件完成时才封闭会话。 */
  private async completeSessionWhenNoPendingFile(
    client: PoolClient,
    uploadSessionId: string,
  ): Promise<void> {
    await client.query(
