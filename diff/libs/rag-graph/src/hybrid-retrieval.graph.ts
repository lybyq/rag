...
const RetrievalState = new StateSchema({
  ...
  /** Evidence 扩展后失效时携带上一轮计划，只用于构造受限第二轮，不复用旧权限 Filter。 */
  forcedRetryPlan: RetrievalPlanSchema.optional(),
...
/** 调用编译后 查询规划与混合检索 图时必须提供、且不能依赖节点默认值的初始状态。 */
export interface HybridRetrievalGraphInput {
  ...
  /** 省略表示正常首轮；提供时从该计划构造且只执行第二轮受限查询。 */
  readonly forcedRetryPlan?: RetrievalPlan;
}

/** 对外隐藏 LangGraph 复杂泛型，只暴露 查询规划与混合检索 需要的编译图执行能力。 */
export interface CompiledHybridRetrievalGraph {
  invoke(input: HybridRetrievalGraphInput): Promise<RetrievalStateValue>;
}

/** Graph 外部依赖与远程调用预算。 */
export interface HybridRetrievalGraphDependencies {
...
/** 构造并编译 查询规划与混合检索 LangGraph；编译会检查孤立节点与边定义。 */
export function createHybridRetrievalGraph(
  dependencies: HybridRetrievalGraphDependencies,
): CompiledHybridRetrievalGraph {
  ...
  const planNode: GraphNode<typeof RetrievalState> = timedNode(
    ...
    (state) => {
      ...
      const firstPlan = buildRetrievalPlan({
      ...
      const plan = state.forcedRetryPlan
        ? buildRetrievalPlan({
            question: buildSecondRoundQuestion(state.forcedRetryPlan),
            route,
            literals: state.literals,
            historyEntities: state.historyEntities,
            filter,
      ...
      return {
        filter,
        plan,
        // 受限重检索已经使用固定规则构造第二轮问题，不再重复调用一次查询改写 LLM。
        needsRewrite:
          !state.forcedRetryPlan &&
          shouldUseLlmRewrite(state.question, route, state.historyEntities),
...
/**
 * 查询规划与混合检索 应用入口：普通 retrieve 供 证据与答案生成 复用；debug 额外要求管理员/审计角色并移除正文。
 */
export class HybridRetrievalService {
  private readonly graph: ReturnType<typeof createHybridRetrievalGraph>;
  ...
  public constructor(
    ...
    this.graph = createHybridRetrievalGraph(dependencies);
  }

  /** 执行真实检索子图；只允许 Run owner，且执行前按当前身份重新收窄空间。 */
  public async retrieve(context: AccessContext, runId: string): Promise<HybridRetrievalResult> {
    return this.execute(context, runId);
  }

  /**
   * Evidence 回源后候选全部失效时执行一次真正的受限第二轮。
   *
   * 它重新鉴权、重新编译 Filter、重新向量化并检索，但从上一计划构造 round=2 查询；不会重复
   * 首轮可选 LLM 改写，也不会突破 Run 快照的最多两轮和总 Deadline。
   *
   * @requirement OPT-004
   */
  public async retryAfterEvidenceFailure(
    context: AccessContext,
    runId: string,
    previous: HybridRetrievalResult,
  ...
  private async execute(
    ...
    const state = await this.graph.invoke({
      ...
      ...(forcedRetryPlan ? { forcedRetryPlan } : {}),
    });
    const route = requireRoute(state.route);
    return {
      question,
...
function requireUniformProfile(
  ...
  // profile 参数从 会话运行与事件 Run 快照透传，不能在图执行中读取热更新环境变量。
