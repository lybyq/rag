...
export const AppEnvironmentSchema = z
  .object({
    ...
    PARSER_REVISION: z.string().min(1).max(100).default('1.1.0'),
    ...
    PARSER_MAX_CONCURRENCY: z.coerce.number().int().min(1).max(16).default(2),
    ...
    FILE_MAX_EXPANDED_TABLE_CELLS: z.coerce
      .number()
      .int()
      .min(1_000)
      .max(200_000_000)
      .default(10_000_000),
    FILE_MAX_TABLE_ROWS: z.coerce.number().int().min(10).max(1_000_000).default(100_000),
    FILE_MAX_TABLE_COLUMNS: z.coerce.number().int().min(10).max(100_000).default(16_384),
    FILE_MAX_TABLE_SPAN: z.coerce.number().int().min(1).max(100_000).default(10_000),
    FILE_MAX_OUTPUT_CHARACTERS: z.coerce
      .number()
      .int()
      .min(1_000_000)
      .max(1_000_000_000)
  ...
  .superRefine((value, context) => {
    ...
    if (value.FILE_MAX_EXPANDED_TABLE_CELLS < value.FILE_MAX_TABLE_CELLS) {
      context.addIssue({
        code: 'custom',
        path: ['FILE_MAX_EXPANDED_TABLE_CELLS'],
        message: '表格展开单元格上限不能小于实际单元格上限',
      });
    }
  ...
  return Object.freeze({
    ...
    fileProcessing: Object.freeze({
      ...
      parser: Object.freeze({
        ...
        maxConcurrency: value.PARSER_MAX_CONCURRENCY,
      ...
      limits: Object.freeze({
        ...
        maxExpandedTableCells: value.FILE_MAX_EXPANDED_TABLE_CELLS,
        maxTableRows: value.FILE_MAX_TABLE_ROWS,
        maxTableColumns: value.FILE_MAX_TABLE_COLUMNS,
        maxTableSpan: value.FILE_MAX_TABLE_SPAN,
        maxOutputCharacters: value.FILE_MAX_OUTPUT_CHARACTERS,
