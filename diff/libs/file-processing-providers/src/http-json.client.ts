 ...
 * 成功响应和错误响应都受字节上限保护；符合平台公开错误契约的稳定 code 会跨服务保留，
 * 但供应商 message 不向业务层透传，避免正文、URL 或内部堆栈泄漏。
 *
 * @requirement PAR-024
 */
...
async function postOnce(
  ...
  if (!response.ok) {
    const publicError = await readPublicProviderError(
      response,
      Math.min(config.maxResponseBytes, 64 * 1024),
      combined,
    );
    const retryable = publicError?.retryable ?? (response.status === 429 || response.status >= 500);
    ...
    throw new ProcessingProviderError(
      failureClass,
      publicError?.code ?? `PROVIDER_HTTP_${response.status}`,
...
async function readPublicProviderError(
  response: Response,
  maxBytes: number,
  signal: AbortSignal,
): Promise<{ readonly code: string; readonly retryable: boolean } | undefined> {
  ...
  try {
    ...
    const retryable = (candidate as { readonly retryable?: unknown }).retryable;
    if (
      typeof code !== 'string' ||
