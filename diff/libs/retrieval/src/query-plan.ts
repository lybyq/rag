 ...
 * 判断确定性第二轮是否真的新增了查询文本。
 *
 * 只忽略大小写和空白差异；不做模糊语义合并，避免把业务上不同的金额、日期或专有名词误判
 * 为相同问题。没有新增内容时重复请求 Embedding/Milvus 不会提高召回，只会拉长回答时间。
 *
 * @requirement RET-015
 */
export function secondRoundAddsNewQuery(plan: RetrievalPlan): boolean {
  const secondRound = normalizeComparableQuery(buildSecondRoundQuestion(plan));
  const existing = [plan.primaryQuery, ...plan.subQuestions].map(normalizeComparableQuery);
  return secondRound.length > 0 && !existing.includes(secondRound);
}
