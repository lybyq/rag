...
export interface CreateUploadFileCommand extends UploadFileDescriptor {
  ...
  /** 外部来源系统稳定 ID；仅外部批次存在，并与 externalDocumentId 成对。 */
  readonly externalSourceId?: string;
  /** 外部来源内稳定文档 ID；用于内容更新时复用本平台 Document。 */
  readonly externalDocumentId?: string;
}

/** 外部批次的请求级幂等事实；Hash 来自规范化后的完整 Body。 */
export interface ExternalBatchIdempotency {
...
export interface CreateUploadSessionCommand {
  ...
  readonly externalBatch?: ExternalBatchIdempotency;
}

/** 已持久化的外部批次幂等绑定。 */
export interface ExistingDocumentBatch {
...
export interface DocumentIngestionRepository {
  ...
  findDocumentBatchByIdempotency(
    context: AccessContext,
    spaceId: string,
    idempotencyKey: string,
  ): Promise<ExistingDocumentBatch | undefined>;
