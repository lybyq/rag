...
/** Worker 可观测的稳定处理结果。 */
export type IndexingOutcome = 'PUBLISHED' | 'REBASE_QUEUED' | 'STAGED' | 'NOT_CLAIMABLE' | 'FAILED';
...
export class IndexingService {
  ...
  public async process(
    ...
    try {
      ...
      // OPT-012：重建使用同一个、尚未对外可见的 Manifest ID。先确保 Collection 存在，
      // 再按 manifestId 删除上一次构建留下的向量；否则已经不属于新快照的旧向量会让对账失败。
      ...
      if (build.resetVectorRecordsBeforeBuild) {
      ...
      const publication = await this.repository.publish(build.run.id, jobId, workerId);
