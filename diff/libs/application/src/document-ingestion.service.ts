...
export class DocumentIngestionService {
  ...
  public async createUploadSession(
    ...
    return this.createUploadSessionInternal(context, request);
  }

  /**
   * 创建可被外部系统安全重试的导入批次。
   *
   * Header 幂等键绑定规范化请求 Hash：同键同请求返回原 batch，同键不同请求报 409。并发
   * 首次请求由 PostgreSQL 唯一约束裁决，失败一方会清理自己初始化的 Multipart 后读取胜者。
   *
   * @requirement OPT-013
   */
  public async createDocumentBatch(
    ...
    this.assertUploadLimits({ spaceId, files: request.files });
    const requestSha256 = documentBatchRequestSha256(spaceId, request);
    const existing = await this.repository.findDocumentBatchByIdempotency(
      context,
      spaceId,
      idempotencyKey,
    );
    if (existing) {
      this.assertBatchRequestHash(existing.requestSha256, requestSha256);
      const uploadSession = await this.getUploadSession(context, existing.uploadSessionId);
      return { batchId: uploadSession.id, uploadSession, replayed: true };
    }

    const externalBatch: ExternalBatchIdempotency = { idempotencyKey, requestSha256 };
    try {
      const uploadSession = await this.createUploadSessionInternal(
        context,
        { spaceId, files: request.files },
        externalBatch,
      );
      return { batchId: uploadSession.id, uploadSession, replayed: false };
    } catch (error: unknown) {
      // 两个相同请求同时首次到达时，一方会命中数据库唯一约束。重新读取赢家而不是让调用方
      // 获得随机 409；createUploadSessionInternal 已经补偿本方初始化的 Multipart。
      if (!(error instanceof ApplicationError) || error.code !== 'DUPLICATE_RESOURCE') throw error;
      const winner = await this.repository.findDocumentBatchByIdempotency(
        context,
        spaceId,
        idempotencyKey,
      );
      if (!winner) throw error;
      this.assertBatchRequestHash(winner.requestSha256, requestSha256);
      const uploadSession = await this.getUploadSession(context, winner.uploadSessionId);
      return { batchId: uploadSession.id, uploadSession, replayed: true };
    }
  }

  /** 创建普通或外部幂等上传会话的共享实现。 */
  private async createUploadSessionInternal(
    context: AccessContext,
    request: CreateUploadSessionRequest,
    externalBatch?: ExternalBatchIdempotency,
  ): Promise<UploadSession> {
   ...
   * 返回批次内每个文件的真实处理状态。
   *
   * batchId 直接复用上传会话 ID；已完成文件读取其幂等完成事实和最新 Job，未完成文件不伪造
   * jobId。最多 100 项且限制 5 路查询，避免一个批次把 PostgreSQL 连接池打满。
   *
   * @requirement OPT-013
   */
  public async getDocumentBatch(
    context: AccessContext,
    batchId: string,
  ): Promise<DocumentBatchStatus> {
    const session = await this.getUploadSession(context, batchId);
    const files = await this.repository.listUploadFiles(context, batchId);
    const statuses = await mapWithConcurrency(files, 5, async (file) => {
      if (!file.externalSourceId || !file.externalDocumentId) {
        // 普通 Upload Session 不能伪装成外部批次；否则调用方会误把临时 clientFileId 当长期主键。
        throw new ApplicationError('SCHEMA_MISMATCH', 500, '批次缺少外部文档身份');
      }
      const completed =
        file.fileStatus === 'COMPLETED'
          ? await this.repository.getCompletedUploadResult(context, file.id)
          : undefined;
      return {
        fileId: file.id,
        ...
        documentId: completed?.document.id ?? null,
        documentVersionId: completed?.documentVersion.id ?? null,
        jobId: completed?.job.id ?? null,
        jobStatus: completed?.job.status ?? null,
        currentStep: completed?.job.currentStep ?? null,
        overallPercent: completed?.job.overallPercent ?? null,
        publicMessage: completed?.job.publicMessage ?? null,
      };
    });
    return { batchId, spaceId: session.spaceId, status: session.status, files: statuses };
  }
  ...
  public async completeUpload(
    ...
    if (result.file.objectKey !== file.objectKey) {
      // 外部业务把同一内容放进了新批次时，Repository 会返回原 Document/Version/File/Job。
      // 新隔离对象已没有业务引用，尽力删除即可；删除失败不能让调用方误以为数据库提交失败，
      // 否则反复重试只会制造更多对象。
      await this.storage
        .removeObject(file.bucket, file.objectKey, this.callOptions())
        .catch(() => undefined);
    }
    await this.audit.append(context, {
  ...
  /** 同一个幂等键不能在之后悄悄改文件列表、外部主键或内容 Hash。 */
  private assertBatchRequestHash(expected: string, actual: string): void {
    if (expected !== actual) {
      throw new ApplicationError('DUPLICATE_RESOURCE', 409, '幂等键已绑定到不同的批次请求');
    }
  }
 ...
 * 对外部批次请求生成稳定 Hash。
 *
 * 不直接 JSON.stringify 原始对象，避免调用方字段顺序差异产生不同 Hash；文件顺序保留，因为它
 * 决定返回计划顺序。这里只写摘要，不写文件正文。
 */
function documentBatchRequestSha256(spaceId: string, request: CreateDocumentBatchRequest): string {
  const canonical = {
    spaceId,
...
/** 小型并发池：批量状态允许部分文件独立读取，但不无界占用数据库连接。 */
