...
export interface CompleteDocumentProcessingCommand {
  ...
  /** 低基数目标结果计数，供运行详情和告警使用；不得把 targetId 放入指标键。 */
  readonly ocrOutcomeCounts: Readonly<Record<string, number>>;
  /** true 表示解析事实可保存，但不允许继续投递 Chunk/索引。 */
  readonly requiresManualReview: boolean;
  /** 人工审核原因只使用稳定公开消息，不保存供应商原始响应或正文。 */
