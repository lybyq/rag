...
export interface IndexBuildInput {
  ...
  /**
   * true 表示同一 Run 正在基于更新后的空间 Head 重建。
   * Application 必须先删除该不可见 Manifest 的旧向量，避免旧主键残留导致对账失败。
   */
  readonly resetVectorRecordsBeforeBuild?: boolean;
...
export type PublishManifestResult =
  ...
  | {
      /** 当前候选基线已过期；Repository 已可靠释放 Job 并写入自动重建 Outbox。 */
      readonly outcome: 'REBASE_QUEUED';
      readonly staleManifestId: string;
