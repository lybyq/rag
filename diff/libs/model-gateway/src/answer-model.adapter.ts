  ...
  choices: z
    .array(
      z.object({
        ...
        message: z.object({
          // GLM/vLLM 开启推理时可能单独返回 reasoning_content；答案 JSON 只允许从 content 读取。
  ...
  public async generateDraft(
    ...
    if (this.config.llm.adapter === 'http') {
      ...
      draft = await this.requestOpenAi<AnswerDraft>(
        ...
        AnswerDraftSchema,
        options,
        this.config.llm.generationMaxOutputTokens,
      );
    }
  ...
  public async rerankEvidence(
    ...
    if (this.config.llm.adapter === 'http') {
      ...
      result = await this.requestOpenAi(
        ...
        options,
        this.config.llm.rerankMaxOutputTokens,
      );
    }
  ...
  public async judgeGrounding(
    ...
    if (this.config.llm.adapter === 'http') {
      ...
      const decision = await this.requestOpenAi(
        ...
        options,
        this.config.llm.judgeMaxOutputTokens,
      );
  ...
  private async requestOpenAi<T>(
    ...
    const payload = await this.requestJson(
      joinUrl(this.config.llm.baseUrl, 'chat/completions'),
      {
        ...
        ...(this.config.llm.jsonMode === 'response-format'
    ...
    // `length`/内容过滤等非 stop 终止都意味着 JSON 可能不完整，必须与一般 Schema 错误分开统计。
    if (choice?.finish_reason && choice.finish_reason !== 'stop') {
      throw new AnswerModelProviderError('PARTIAL_RESULT', false);
    }
    // reasoning_content 是模型私有推理，不拼进 content，也不参与事实引用或 JSON 修补。
    if (!choice?.message.content) throw new AnswerModelProviderError('SCHEMA_ERROR', false);
    return parseJsonContent(choice.message.content, schema);
  }
...
export class FixtureAnswerModelAdapter implements AnswerModelPort {
  ...
  public generateDraft(
    ...
    const sourceWindow = input.context.sourceWindows.find((item) => item.sourceId === sourceId);
    if (!sourceId) {
      return Promise.reject(new AnswerModelProviderError('SCHEMA_ERROR', false));
    }
    const directText = sourceWindow?.content.trim().slice(0, 500) || '企业知识来源内容不可用。';
