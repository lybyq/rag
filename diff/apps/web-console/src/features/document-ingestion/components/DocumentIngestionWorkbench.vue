...
/** 最新完成上传的任务会自动打开详情；多文件的其余任务仍保留在任务列表中逐个查看。 */
const latestJobId = shallowRef('');
...
<template>
  <div class="ingestion-workbench">
    ...
    <UploadWorkbench
      v-model:space-id="selectedSpaceId"
      :spaces="spaces"
      @job-created="latestJobId = $event"
    />
    <DocumentLibrary :spaces="spaces" />
    <JobCenter :spaces="spaces" :focus-job-id="latestJobId" />
  </div>
</template>
