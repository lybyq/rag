...
/** 一条由后端真实节点事件生成的用户可见处理说明。 */
export interface ProcessingTimelineItem {
  readonly sequence: number;
  readonly phase: RagRunProgressPayload['phase'];
  readonly status: RagRunProgressPayload['status'];
...
export interface RagChatComposable {
  ...
  readonly timeline: ShallowRef<readonly ProcessingTimelineItem[]>;
  readonly elapsedSeconds: ShallowRef<number>;
  readonly slowNoticeVisible: ComputedRef<boolean>;
...
export function useRagChat(): RagChatComposable {
  ...
  const timeline = shallowRef<readonly ProcessingTimelineItem[]>([]);
  ...
  const slowNoticeVisible = computed(
    () =>
      running.value && elapsedSeconds.value >= (currentRun.value?.snapshot.slowNoticeSeconds ?? 12),
  );
  ...
  async function applyEvent(event: RagRunEvent): Promise<void> {
    const progress =
      event.eventType === 'run.progress'
        ? RagRunProgressPayloadSchema.safeParse(event.payload)
        : undefined;
    if (progress?.success) {
      const message = authorizedProgressMessage(progress.data, spaces.value);
      stage.value = message;
      timeline.value = [
        ...timeline.value,
        {
          ...
          ...(progress.data.durationMs === undefined
            ? {}
            : { durationMs: progress.data.durationMs }),
        },
      ].slice(-12);
    } else {
  ...
  onMounted(() => {
    ...
    elapsedTimer = window.setInterval(() => {
      if (!running.value || !currentRun.value) {
        elapsedSeconds.value = 0;
        return;
      }
      const startedAt = currentRun.value.startedAt ?? currentRun.value.createdAt;
 ...
 * 生成检索阶段的公开说明。
 *
 * 空间名称只用“后端本次 Run 已授权 ID”和“页面当前仍可见空间”求交；任一 ID 已撤权或页面
 * 未知时回退服务端脱敏文案，绝不拿提交时的空间名称补齐，防止 SSE 重连期间泄漏撤权对象。
 *
 * @requirement OPT-011
 */
export function authorizedProgressMessage(
  progress: RagRunProgressPayload,
  spaces: readonly KnowledgeSpace[],
): string {
  if (progress.phase !== 'RETRIEVAL' || !progress.authorizedSpaceIds?.length) {
    return progress.publicMessage;
  }
  const authorizedIds = new Set(progress.authorizedSpaceIds);
  const names = spaces.filter((space) => authorizedIds.has(space.id)).map((space) => space.name);
  return names.length === progress.authorizedSpaceIds.length
    ? `正在检索你选择的“${names.join('、')}”知识空间。`
