...
import { WorkerDocumentParserExecutor } from './parser-executor.service';
import { ParserSourceLoader } from './source-loader.service';
import { DOCUMENT_PARSER_EXECUTOR, DOCUMENT_PARSER_REGISTRY } from './tokens';
...
@Module({
  ...
  providers: [
    ...
    {
      provide: DOCUMENT_PARSER_EXECUTOR,
      inject: [APP_CONFIG],
      useFactory: (config: AppConfig): WorkerDocumentParserExecutor =>
        new WorkerDocumentParserExecutor(
          {
            ...config.fileProcessing.limits,
            maxInputBytes: config.fileProcessing.parser.maxInputBytes,
            maxArchiveEntries: config.fileProcessing.parser.maxArchiveEntries,
          ...
          config.fileProcessing.parser.maxConcurrency,
        ),
    },
  ],
})
