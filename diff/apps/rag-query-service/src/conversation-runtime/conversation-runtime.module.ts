/**
 * 会话运行与事件 rag-query-service Composition Root。
 *
 * 本模块复用 身份权限与知识空间 的三种认证 Adapter 与授权服务，组装 PG 事实源、Redis Stream、
  ...
  providers: [
    ...
    },
    {
      ...
      useFactory: (
        ...
        config: AppConfig,
        featureFlags: FeatureFlagResolverPort,
      ): RagRunService =>
        new RagRunService(
          repository,
          authorization,
          ...
          cancellation,
          {
            ...
            slowNoticeSeconds: config.answer.slowNoticeSeconds,
            retrieval: {
