/**
 * 索引构建与发布 真实 PostgreSQL + 内存向量库集成门禁。
 *
 * 本测试用公开合成 Chunk 验证：知识加工与质量→索引构建与发布 交接、Embedding 事实复用、Manifest 原子发布、
...
describeWithInfra(
  ...
  () => {
    ...
    const owner: AccessContext = {
      user: createTestUserContext(`indexing-publication-owner-${suffix}`, [
        'KNOWLEDGE_EDITOR',
        'SYSTEM_ADMIN',
      ]),
      requestId: `indexing-publication-request-${suffix}`,
      traceId: `indexing-publication-trace-${suffix}`,
    };
    const profile: EmbeddingProfile = {
      profileId: `indexing-publication-integration-${suffix}`,
    ...
    const rolloutProfile: EmbeddingProfile = {
      ...profile,
      profileId: `indexing-publication-rollout-${suffix}`,
      revision: `2-${suffix}`,
    };
    let spaceId = '';
    ...
    beforeAll(async () => {
      spaceId = (
        await spaces.create(owner, {
          code: `indexing-publication-it-${suffix}`,
          name: '索引构建与发布 集成测试空间',
          description: null,
    ...
    it('Milvus 构建失败不会改变当前 ACTIVE Head，之后仍可一键回滚历史版本', async () => {
      ...
      const failed = await seedKnowledgeProcessingPassedDocument(
        pool,
        spaceId,
        owner.user.userId,
        suffix,
    ...
    it('新 Profile 自动构建候选、离线评测、登记 CANARY，并可提升和请求级回退', async () => {
      ...
      await markRebuildKnowledgeProcessingPassed(pool, rebuildJobId, suffix);
      ...
      const rollout = new ProfileRolloutService(
        indexing,
        embeddingFixture(rolloutProfile),
        vector,
        {
    ...
    it('撤权和空间废止均在同一事务可靠产生缓存失效事件', async () => {
      const grant = await spaces.upsertGrant(owner, spaceId, {
        subjectType: 'ROLE',
        subjectId: `indexing-publication-reader-${suffix}`,
        permissions: ['READ'],
        reason: '集成测试临时授权',
      });
...
/** 构造一个已通过 知识加工与质量、当前等待 EMBED 的最小真实数据库快照。 */
async function seedKnowledgeProcessingPassedDocument(
  pool: Pool,
  spaceId: string,
  userId: string,
  suffix: string,
  ...
  const jobId = `indexing-publication:${suffix}:${sequence}:${randomUUID()}`;
  const parseRunId = randomUUID();
  const processingRunId = randomUUID();
  const parentId = `indexing-publication-parent-${suffix}-${sequence}`;
  const childId = `indexing-publication-child-${suffix}-${sequence}`;
  ...
  try {
    await client.query(
      ...
      [documentId, spaceId, `索引构建与发布 合成文档 ${sequence}`, userId],
    );
    await client.query(
    ...
    await client.query(
      ...
      [
        versionId,
        `indexing-publication-${sequence}.txt`,
        `integration/${spaceId}/${versionId}.txt`,
        sha,
      ],
    );
    await client.query(
...
/** 为 prepareProfileRebuild 创建的标准 Job 补出合成 文件解析与OCR/知识加工与质量 成功事实。 */
async function markRebuildKnowledgeProcessingPassed(
  pool: Pool,
  jobId: string,
  suffix: string,
): Promise<void> {
  ...
  try {
    ...
    const parseRunId = randomUUID();
    const processingRunId = randomUUID();
    const parentId = `indexing-publication-rollout-parent-${suffix}`;
    const childId = `indexing-publication-rollout-child-${suffix}`;
