/**
 * 真实 PostgreSQL 验证 文件解析与OCR→知识加工与质量 Outbox 交接、Chunk/质量原子提交、审核乐观锁、越权拒绝和新 revision。
...
describeWithInfra(
  ...
  () => {
    ...
    const owner: AccessContext = {
      user: createTestUserContext(`knowledge-processing-owner-${suffix}`, ['KNOWLEDGE_EDITOR']),
      requestId: `knowledge-processing-owner-request-${suffix}`,
      traceId: `knowledge-processing-trace-${suffix}`,
    };
    const intruder: AccessContext = {
      user: createTestUserContext(`knowledge-processing-intruder-${suffix}`, [
        'KNOWLEDGE_REVIEWER',
      ]),
      requestId: `knowledge-processing-intruder-request-${suffix}`,
    };
    let spaceId = '';
    ...
    beforeAll(async () => {
      const space = await spaces.create(owner, {
        code: `knowledge-processing-it-${suffix}`,
        name: '知识加工与质量 集成测试空间',
        description: null,
    ...
    it('文件解析与OCR 事务产生 知识加工与质量 Outbox，PASS 后只有非重复 Child 获得索引资格', async () => {
      ...
      const outcome = await runKnowledgeProcessing(prepared.jobId);
    ...
    it('并发审核只有一个 expectedVersion 能成功，且无 ACL 的审核者被默认拒绝', async () => {
      ...
      expect(await runKnowledgeProcessing(prepared.jobId)).toBe('MANUAL_REVIEW');
    ...
    async function runKnowledgeProcessing(jobId: string): Promise<string> {
      const workerId = `knowledge-processing-worker-${randomUUID()}`;
      ...
      if (outcome === 'FAILED') {
        ...
        throw new Error(`知识加工与质量 集成处理失败：${failedJob?.publicMessage ?? 'unknown'}`);
      }
      return outcome;
    }
    ...
    async function prepareParsedJob(
      ...
      const workerId = `document-parsing-worker-${randomUUID()}`;
      ...
      const parseRun = await parsing.beginRun({
        input: input!,
        providerProfile: 'external-ci',
        parserProfileId: 'knowledge-processing-integration-parser',
        parserRevision: '1',
        ocrProfileId: 'knowledge-processing-integration-ocr',
        ocrRevision: '1',
      });
      const malware = {
      ...
      const parser = {
        ...
        blocks: candidates,
        pages: [
          { pageNo: 1, textCharacterCount: text.length, textCoverage: 0.8, imageOnly: false },
        ],
        ocrCandidates: [],
        inspection: {
          encrypted: false,
        ...
        files: [
          {
            ...
            originalFileName: 'knowledge-processing-synthetic.pdf',
            strategy: 'SINGLE',
