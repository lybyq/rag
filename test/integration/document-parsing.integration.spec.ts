/** 真实 PostgreSQL 验证 文件解析与OCR lease fencing、Block 原子提交和管理员查询。 */
...
describeWithInfra('[PAR-012][PAR-013][PAR-015] 文件解析与OCR PostgreSQL transaction', () => {
  ...
  const context: AccessContext = {
    user: createTestUserContext(`document-parsing-owner-${suffix}`, ['KNOWLEDGE_EDITOR']),
  ...
  beforeAll(async () => {
    const space = await spaces.create(context, {
      code: `document-parsing-it-${suffix}`,
      name: '文件解析与OCR 集成测试空间',
      description: null,
  ...
  it('只有 lease owner 能原子写入 Block，完成后通过 Outbox 排队 知识加工与质量 CHUNK', async () => {
    ...
    const workerId = `document-parsing-worker-${suffix}`;
      ...
      files: [
        {
          ...
          originalFileName: 'document-parsing-golden.pdf',
          strategy: 'SINGLE',
