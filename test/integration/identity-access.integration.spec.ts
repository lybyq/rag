...
describeWithInfra(
  '[AUTH-007][AUTH-012][AUTH-015] 身份权限与知识空间 PostgreSQL transaction',
  () => {
    ...
    const adminContext: AccessContext = {
      ...
      requestId: `identity-access-integration-${suffix}`,
    };
    ...
    it('创建 owner ACL，授权/撤权生成版本并递增全局 authzVersion', async () => {
      ...
      const space = await repository.create(adminContext, {
        code: `identity-access-it-${suffix}`,
        name: '身份权限与知识空间 集成测试空间',
        description: null,
