/**
 * 会话运行与事件 真实 PostgreSQL + Redis 集成门禁。
 *
...
import { Pool } from 'pg';
import { AesGcmSensitiveTextProtector } from '../../apps/rag-query-service/src/conversation-runtime/aes-gcm-sensitive-text.protector';

const describeWithInfra = process.env.RUN_INTEGRATION_TESTS === 'true' ? describe : describe.skip;

describeWithInfra('[RUN-001..014] 会话运行与事件 conversation and run', () => {
  ...
  const protector = new AesGcmSensitiveTextProtector({
    ...
    contentEncryptionKey: Buffer.from('run-encryption-key-32-bytes-0000').toString('base64'),
  });
  const suffix = randomUUID().slice(0, 8);
  const spaceId = randomUUID();
  const manifestId = randomUUID();
  const profileId = `conversation-runtime-profile-${suffix}`;
  const owner: AccessContext = {
    user: createTestUserContext(`conversation-runtime-owner-${suffix}`, ['KNOWLEDGE_READER']),
    requestId: `conversation-runtime-request-${suffix}`,
    traceId: `conversation-runtime-trace-${suffix}`,
  };
  const outsider: AccessContext = {
    user: createTestUserContext(`conversation-runtime-outsider-${suffix}`, ['KNOWLEDGE_READER']),
    requestId: `conversation-runtime-outsider-request-${suffix}`,
  };
  ...
  const service = new RagRunService(repository, authorization, protector, stream, stream, {
    flowVersion: 'conversation-runtime-flow-v1',
    policyVersion: 'conversation-runtime-policy-v1',
    promptProfileId: 'conversation-runtime-prompt-v1',
    validatorProfileId: 'conversation-runtime-validator-v1',
    embeddingProfileId: profileId,
    embeddingRevision: 'embedding-r1',
    rerankerProfileId: 'conversation-runtime-reranker',
  ...
  beforeAll(async () => {
    ...
    conversationId = (await service.createConversation(owner, { title: '会话运行与事件 集成会话' }))
      .id;
  });
  ...
  afterAll(async () => {
    ...
    await cleanupConversationRuntime(pool, conversationId, spaceId, profileId);
    await pool.end();
  });

  test('[RUN-002][RUN-004] 并发重放只生成一份 Run/消息且冻结发布与模型版本', async () => {
    const key = `conversation-runtime-concurrent-${suffix}`;
  ...
  test('[RUN-005..012] 答案事实先于顺序完成事件，Ticket 只能兑换一次且撤权后历史脱敏', async () => {
    const accepted = await service.createRun(
      owner,
      conversationId,
      `conversation-runtime-complete-${suffix}`,
      {
        question: '制度的报销上限是什么？',
  ...
  test('[RUN-010] 取消先中止 Signal，再由执行器确认 CANCELLED，其他用户始终得到 404', async () => {
    const accepted = await service.createRun(
      owner,
      conversationId,
      `conversation-runtime-cancel-${suffix}`,
      {
        question: '请取消这个问题',
        requestedSpaceIds: [spaceId],
      },
  ...
  test('[RUN-005][RUN-014] Deadline 转 EXPIRED，正文到期只脱敏而保留审计 Hash', async () => {
    const accepted = await service.createRun(
      owner,
      conversationId,
      `conversation-runtime-expire-${suffix}`,
      {
        question: '即将到期的合成问题',
        requestedSpaceIds: [spaceId],
      },
...
/** 创建 会话运行与事件 需要的最小 ACTIVE 索引构建与发布 发布事实，不伪造任何检索结果。 */
async function seedPublishedSpace(pool: Pool, input: SeedPublishedSpaceInput): Promise<void> {
  const compatibility = createHash('sha256')
    .update(`conversation-runtime-${input.suffix}`)
    .digest('hex');
  const collection = `conversation_runtime_collection_${input.suffix}`;
  const alias = `conversation_runtime_alias_${input.suffix}`;
  await pool.query(
    `INSERT INTO knowledge_spaces (id, code, name, owner_user_id)
     VALUES ($1,$2,'会话运行与事件 集成空间',$3)`,
    [input.spaceId, `conversation-runtime-it-${input.suffix}`, input.ownerUserId],
  );
  await pool.query(
    `INSERT INTO knowledge_space_policies (space_id, version, grants, changed_by, change_reason)
     VALUES ($1,1,'[]'::jsonb,$2,'会话运行与事件 集成测试基线')`,
  ...
  await pool.query(
    `INSERT INTO embedding_collection_registry (
     ...
     ) VALUES ($1,$2,'external-ci','conversation-runtime-fixture','embedding-r1','tokenizer-r1',2,true,
  ...
  await pool.query(
    `INSERT INTO space_manifests (
     ...
     ) VALUES ($1,$2,1,'ACTIVE','external-ci',$3,'conversation-runtime-fixture','embedding-r1',
...
async function publishUntilEmpty(publisher: RagRunEventPublisherService): Promise<void> {
  for (let round = 0; round < 20; round += 1) {
    const result = await publisher.publishBatch(`conversation-runtime-it-publisher-${round}`);
    expect(result.failed).toBe(0);
    if (result.claimed === 0) return;
  }
  throw new Error('会话运行与事件 Outbox 在限定轮次内未排空');
}
...
async function cleanupConversationRuntime(
  pool: Pool,
