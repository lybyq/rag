...
安全结论为 REJECTED 或 MANUAL_REVIEW 时不会进入 知识加工与质量。成功路径写版本化 `blocks.json`，SHA 相同可复用；PG `complete()` 在一个事务内写 Block、Issue、Run 和任务步骤。每次提交前再次检查 lease，旧 Worker 即使晚到也不能覆盖新 Worker。
