...
本项目缓存与临时目录应放 D 盘。命令执行前设置 `TEMP/TMP=D:\codex-temp\rag-document-parsing`，`PARSER_TEMP_ROOT` 使用 D 盘项目目录；Docker data-root 也应迁到有容量的数据盘。未经确认不要在已满 C 盘拉取可选 Docling OCR 镜像。
