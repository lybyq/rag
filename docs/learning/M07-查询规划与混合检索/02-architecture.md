...
```mermaid
flowchart LR
  A["M06 加密问题 + Run 快照"] --> B["route_query"]
  ...
  J --> E
  I -->|满足或 round=2| K
  K --> L["M08 Evidence/Rerank/Generation"]
```
...
| Graph            | `libs/rag-graph/src/m07-retrieval.graph.ts` | 节点、边、状态、上限        | HTTP、具体 SDK |
| Adapter          | `model-gateway/persistence-*`               | HTTP、Redis、PG、Milvus     | 业务路线选择   |
| HTTP             | `apps/rag-query-service/src/m07`            | 可信输入/输出映射           | 解密、权限规则 |
...
必须创建新 Collection/Manifest；M06 创建 Run 时会拒绝不兼容的索引快照。
