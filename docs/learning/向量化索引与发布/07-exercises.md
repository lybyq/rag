...
不看代码画出 知识加工与质量 PASS 到 Head 切换的顺序，并标出三个“失败但旧版仍在线”的位置。完成后对照 `IndexingService.process()` 和 `publish()`。
...
在本地跑 索引构建与发布 集成测试，暂停在 `markVerified` 后查询 Milvus/Memory 候选和 PG Head。回答为什么“向量存在”和“用户可检索”不是同一事实。
