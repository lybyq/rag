...
新 Profile 建独立 Collection 和 VERIFIED 候选，query 端点对候选做离线自检。CANARY 只登记稳定/候选/比例/salt，不改稳定 Head；查询规划与混合检索 用 userId 稳定 Hash 分桶。确认后 promote 原子切 Head，异常按请求记录的 previous Manifest 回退。
