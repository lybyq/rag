...
## 1. 知识加工与质量 在整条 RAG 链路里的位置

文件解析与OCR 的 `DocumentBlock` 是“解析事实”：它回答原文件有哪些标题、段落、表格、页码和坐标。知识加工与质量 的 `KnowledgeChunk` 是“检索策略产物”：它回答应该把哪些上下文一起送去向量化、哪些内容只能展示、以及一段知识从哪里来。
...
- `SOURCE_BLOCK`：回到 文件解析与OCR 原始 Block、页码、Sheet、Slide 和 bbox；
- `TABLE_HEADER`：表格分段后仍能找回表头；
- `FOOTNOTE`：正文与脚注关系；
- `DUPLICATE_OF`：重复内容的原始出处不丢失。

显式关系是可审计 RAG 的基础。后续 证据与答案生成 展开上下文时可以逐条重新鉴权，而不需要相信一段拼接文本。
...
- `PASS`：自动规则通过，可以把非重复 Child 交给 索引构建与发布；
