...
```mermaid
flowchart LR
  文件解析与OCR["文件解析与OCR CLEAN Block + Outbox"] --> Consumer["Ingestion Consumer"]
  ...
  Gate -->|PASS| 索引构建与发布["索引构建与发布 EMBED 等待"]
  Gate -->|MANUAL_REVIEW| Review["Review API + Console"]
  Gate -->|REJECT| Stop["阻断索引"]
  Review -->|approve| 索引构建与发布
  Review -->|reprocess| 文件解析与OCR
```
...
Chunk、关系、质量报告都绑定该 Run。历史结果不可因当前配置变化而被重新解释。索引构建与发布 只允许读取 `eligible_for_index=true` 的 Child，而资格必须来自自动 PASS 或合法人工批准。

## 4. 事务边界

文件解析与OCR 完成时，在写 Parse Run/Block 的同一事务内把 `CHUNK` 步骤排队并写 `ingestion.knowledge_processing.requested` Outbox。知识加工与质量 完成时，在一个事务内写入 Run、Chunk、关系、质量报告、发现项和任务步骤状态。
...
Worker 延续 文档接入与任务 的 Request/Trace、Inbox 幂等和 Lease fencing，并增加：

- `rag_knowledge_processing_operations_total{result}`；
- `rag_knowledge_processing_duration_seconds{result}`。
