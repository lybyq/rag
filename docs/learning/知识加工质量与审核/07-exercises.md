...
运行 知识加工与质量 PostgreSQL 集成测试，在两个 Promise 提交前打断点，观察行锁和 optimisticVersion。尝试删除行锁或版本检查，看为何会出现覆盖。
...
在 知识加工与质量 处理到 Chunk 后主动终止 Worker，等待 lease 过期并让新 Worker 接管。观察旧 Worker 恢复后提交被拒绝，且数据库只保留一套有效结果。

验收：记录 job、lease owner、run、inbox/outbox、最终状态和指标，证明没有重复事实。

## 练习 7：准备面试白板

不看代码画出 文件解析与OCR Outbox 到 知识加工与质量、三态门禁、审核和 索引构建与发布 的链路，并标出四个事务边界、两个权限检查、一个乐观锁和一个 lease fencing。
