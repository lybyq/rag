...
资格是 PostgreSQL 事实：MANUAL_REVIEW/REJECT 的 Chunk 为 false；合法批准才在同一事务中开放允许的 Child。索引构建与发布 只能查询 eligible rows，不能让消息或前端参数声明“已通过”。SUPPRESSED_DUPLICATE 即便报告通过也保持 false。
