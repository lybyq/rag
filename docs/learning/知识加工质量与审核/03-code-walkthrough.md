...
`apps/ingestion-worker/src/ingestion-queue.consumer.ts` 根据事件类型路由 文件解析与OCR 或 知识加工与质量。Inbox key 带阶段，防止相同 eventId 在不同处理器间误判；Consumer 仍通过已有重投机制保证至少一次投递下的幂等。

## 3. 应用服务如何编排

读 `KnowledgeProcessingService.process()`：

1. Repository 只加载当前 revision、文件解析与OCR 已成功且 lease 属于当前 Worker 的 Block。
...
`apps/platform-api/src/knowledge-processing/knowledge-processing.controller.ts` 只做 Zod 输入解析、可信上下文映射和输出 envelope。API 支持运行历史、详情、Chunk 游标分页和审核。
