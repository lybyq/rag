...
因此 查询规划与混合检索 输出的不是答案，而是一组经过当前事实复核的 `RetrievalCandidate`。答案、引用和拒答在 证据与答案生成。
...
被回滚，或制度尚未到生效日期。查询规划与混合检索 只把 Milvus 的 `vectorId` 当候选线索，最终正文和可见性由
PostgreSQL 单次批量查询确认。

## LangGraph 在哪

真正的图位于 `libs/rag-graph/src/hybrid-retrieval.graph.ts`。它不是架构图或未来占位：
...
会话运行与事件 是“运行容器”，查询规划与混合检索 是“检索子图”，证据与答案生成 会成为“生成与校验主图”。
