...
依次打开 `conversations.controller.ts`、`runs.controller.ts` 和 `conversation-http-utils.ts`：

1. `@CurrentUser()` 的身份来自 身份权限与知识空间 Guard，不读取 body 中的 userId/roles。
2. `parseConversationInput` 把 Zod 错误变成稳定 400，避免未知异常被全局 Filter 当成 500。
...
3. 读取发布路由。缺少 ACTIVE Manifest 就 409，避免 查询规划与混合检索 查询一个不存在的索引。
...
1. `20260823100000_conversations_and_runs.sql`：核心表、外键、唯一约束和索引。
2. `20260823110000_conversation_state_version_and_sources.sql`：已部署数据库前滚增加状态版本和摘要来源。
3. `20260823120000_conversation_summary_hash.sql`：摘要密文完整性 Hash 与形状约束。
4. `20260823130000_conversation_summary_retention.sql`：把敏感派生摘要纳入保留期清理。
