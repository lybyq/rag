# 会话运行与事件 架构：PG 事实、Redis 投影和 SSE 适配
...
RagRunService / RagRunLifecycleService / Publisher / Maintenance
        ...
        └── AuthorizationService ───────────► 身份权限与知识空间 ACL + Redis Cache + PG
```

Domain 只知道状态迁移，不依赖 NestJS、PG、Redis 或模型 SDK。Composition Root 位于 `apps/rag-query-service/src/conversation-runtime/conversation-runtime.module.ts`，这里是唯一可以选择具体 Adapter 和读取配置的地方。
...
POST conversation/runs
  ...
  → 确认会话属于当前 userId
  → 身份权限与知识空间 授权缩小 requestedSpaceIds
...
会话运行与事件 不同步调用 LLM。后续 查询规划与混合检索 Worker 领取 Run，再通过 `RagRunLifecycleService.start` 进入 RUNNING。
