...
1. PG `rag_runs.status/deadline_at` 是否正常；会话运行与事件 本身不执行模型，查询规划与混合检索 Worker 未接入时停在 ACCEPTED 是预期。
2. `rag_run_event_outbox` 是否有 `run.accepted`，`published_at` 是否为空。
3. Query Service 的 `conversation_run_operations_total{operation="event_publish"}` 是否有 failure。
