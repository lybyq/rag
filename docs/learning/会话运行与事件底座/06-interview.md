...
## “会话运行与事件 和 查询规划与混合检索 的边界？”

会话运行与事件 管执行容器、状态、事件、取消、会话和合规；查询规划与混合检索 管查询改写、授权检索、融合、Rerank 和证据。查询规划与混合检索 必须使用 会话运行与事件 冻结 Manifest 和 AbortSignal，不能自己读当前 Head 或创建第二套 Run。
