...
依赖方向仍是 Domain → Application Ports ← Adapters。Platform API 的 身份权限与知识空间 Module 是 Composition Root，只有它知道具体使用 PostgreSQL、Redis 和哪种 Auth Adapter。
...
文档正文、引用预览、历史消息、检索候选和导出都可能绕过主列表过滤。身份权限与知识空间 先建立 `protected_resource_spaces` 反查索引和 `requireResourcePermission`：资源 ID → 当前 spaceId → 当前 ACL。后续模块接入具体资源时复用它，不信任旧 Run 中保存的授权结果。
