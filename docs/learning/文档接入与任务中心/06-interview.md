...
答：签名前检查空间 WRITE；Key 由服务端 UUID 生成；URL 绑定 method/key/expiry；完成接口再次鉴权；对象在隔离 bucket，文件解析与OCR 安全门禁前不会发布检索。
...
答：文档接入与任务 事件量相对低，首要目标是与任务状态同事实源、可查询和可审计。PG bigserial 能支持 Last-Event-ID。会话运行与事件 的高频生成流会使用 Redis Stream，但最终结果仍先持久化。
...
答：文档接入与任务 只建立接入与任务底座。魔数/MIME 交叉验证、恶意软件扫描、可信流式 SHA-256、压缩炸弹和隔离 Parser 属于 文件解析与OCR；质量门禁属于 知识加工与质量；向量发布一致性属于 索引构建与发布。不能因为上传成功就让内容进入检索。
