...
5. `database/migrations/20260816130000_document_ingestion.sql`：看约束如何落到事实源。
...
注意 Hash 是“可用时核对”。普通 S3 Multipart ETag 不是 SHA-256，不能把它冒充内容 Hash。文件解析与OCR 会通过安全读取流计算可信 SHA-256。
...
Consumer 的 `consumeQueuedIngestion` 把 Inbox 收据和状态副作用放在同一事务。文档接入与任务 还没有 文件解析与OCR 安全处理器，所以明确进入 `WAITING`，而不是假装解析成功。
