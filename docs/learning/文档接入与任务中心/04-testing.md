# 文档接入与任务 测试：验证故障窗口，不只验证 Happy Path
...
| PG 集成       | 真实事务、行锁、幂等、Inbox、lease          | `document-ingestion.integration.spec.ts` |
...
真实吞吐和长时间稳定性属于 评测与可靠性 Load/Soak，不应该让单元测试分配巨型内存。
...
pnpm exec jest --runInBand --config test/jest-integration.config.cjs `
  test/integration/document-ingestion.integration.spec.ts
