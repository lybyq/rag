# ADR-012：索引构建与发布 原子索引发布与 Profile Rollout
...
7. `FULL` 评测通过后原子切 Head；`CANARY` 只写 `space_manifest_canaries`，稳定 Head 不变。查询规划与混合检索 按 `routing_salt + userId` 稳定分桶。提升和请求级回退均再次校验 Head，防止覆盖期间的新发布。
