...
- [x] 索引构建与发布 领域/契约/Application/Adapter/Worker/API 均有中文 JSDoc 和需求编号。
- [x] 179 个后端测试通过，索引构建与发布 新增边界均有自动化断言。
- [x] 身份权限与知识空间～索引构建与发布 15 个真实 PostgreSQL/Redis 集成场景通过。
...
- [ ] 查询规划与混合检索 在线检索接入 CANARY 指针并验证 userId 稳定分桶。
