# 身份权限与知识空间 Web Component Map：身份与知识空间

> 在写 `.vue` 文件前冻结组件边界，避免把 API、权限判断、表格和表单堆进一个页面。

## 设计方向

延续 工程与决策基线 的“企业知识控制台”语言：深墨色导航、暖灰工作面、橙色操作强调。身份权限与知识空间 使用一张连续的治理工作台，而不是大量悬浮卡片；空间列表是主视觉，右侧授权抽屉承担上下文操作。状态、权限和策略版本使用克制的标签与等宽数字表达。
...
- 身份权限与知识空间 暂不把页面内筛选提升到全局 Store；当前身份可在后续 会话运行与事件 进入 Pinia。
