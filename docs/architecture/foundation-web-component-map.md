...
└─ AppShell.vue                         # 全局布局、导航和用户区域
   └─ RouterView
      ├─ FoundationOverviewView.vue     # 工程与决策基线 工程基线总览（薄页面）
      │  ├─ PageHeader.vue              # 标题、环境徽标、主操作
      │  ├─ SystemPulse.vue             # 四进程和基础设施状态
      │  ├─ DeliveryRoadmap.vue         # 工程与决策基线～前端体验 实施路径
...
- Pinia 留给跨页面身份、知识空间和全局通知；工程与决策基线 的页面内健康状态不需要提前全局化。
