...
| 需求    | 主要实现                                                   | 自动化/验收证据                              |
| ------- | ---------------------------------------------------------- | -------------------------------------------- |
| DOC-001 | `document-ingestion.ts` + 文档接入与任务 migration         | Zod strict typecheck、migration gate         |
...
文档接入与任务 另暴露低基数 `rag_document_ingestion_operations_total{operation,result}` 指标，覆盖上传会话创建/完成/取消、版本重处理、任务取消、队列去重消费、Outbox 发布和租约恢复；用户、文档和任务 ID 不进入标签。
...
application 文档接入与任务 unit: 4 passed
MinIO adapter contract: 3 passed
文档接入与任务 PostgreSQL integration: 3 passed
文档接入与任务 BullMQ/Redis integration: 1 passed
