...
检索以当前空间 Manifest 中的有效文档成员和 Profile 为准，不把“最近发布的一个文档 revision”误当作整个空间唯一内容 revision。具体表结构在 索引构建与发布 ADR 中冻结。
...
| 阶段                 | 模块                           | 可验证结果                               |
| -------------------- | ------------------------------ | ---------------------------------------- |
| Foundation           | 工程与决策基线～文档接入与任务 | 工程、权限、知识空间、上传和任务事实闭环 |
| Knowledge Production | 文件解析与OCR～索引构建与发布  | 文档从隔离区到审核、索引、发布和回滚闭环 |
| RAG Runtime          | 会话运行与事件～证据与答案生成 | Run、SSE、检索、证据、生成和校验闭环     |
| Product & Production | 评测与可靠性～前端体验         | 前端、评测、压测、运维、安全与总验收闭环 |
...
| 风险                     | 影响                       | 控制措施                                                                         |
| ------------------------ | -------------------------- | -------------------------------------------------------------------------------- |
| 内网认证协议未知         | 无法直接联调登录           | 三模式 Auth Adapter；身份权限与知识空间 用 Mock 完成，内网信息确定后新增契约测试 |
...
| Manifest 粒度设计错误    | 发布新文档后旧文档不可见   | 索引构建与发布 前完成发布语义 ADR 和全空间回归测试                               |
