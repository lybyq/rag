...
| 需求    | 主要实现                                                           | 自动化证据                            |
| ------- | ------------------------------------------------------------------ | ------------------------------------- |
| KNO-001 | `knowledge-processing.ts`、知识加工与质量 migration、PG mapper     | Zod 歧义关系/审核反例、migration gate |
...
| KNO-011 | 知识加工与质量 查询/Chunk/审核 API、管理台四组件                   | OpenAPI gate、Vue 可见行为测试        |
...
知识加工与质量 PostgreSQL integration: 3 passed
身份权限与知识空间～知识加工与质量 integration excluding infra-health: 11 passed
...
全套 integration 中独立的 工程与决策基线 `infra-health` 用例未通过：当前本机 PostgreSQL、两套 Redis 为 up，MinIO 与 Milvus 未启动。知识加工与质量 的三个真实 PostgreSQL 场景全部通过；不把未启动依赖写成通过。

## 3. 关键一致性与安全证据

- 文件解析与OCR 完成、CHUNK 排队和 知识加工与质量 Outbox 在同一 PG 事务中提交。
- 知识加工与质量 begin/complete/fail 使用 lease owner + expiry fencing，过期 Worker 不能提交。
...
知识加工与质量 不直接调用 LLM、Embedding、Reranker、OCR 或 Milvus：OCR 属于 文件解析与OCR，Embedding/Milvus 属于 索引构建与发布，LLM/Reranker 属于后续在线问答模块。这个边界避免知识审核结果尚未通过就写入正式向量索引。
...
- 索引构建与发布 完成后验证只有 eligible Child 被向量化，发布前后检索可见性正确。

这些不影响 知识加工与质量 代码需求已落地，但属于企业上线前的跨模块/真实环境门禁，不能用合成 Fixture 替代。
