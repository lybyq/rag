-- M07：混合检索回源字段、可直接批量关联的 vector_id，以及旧 Run 的检索参数快照回填。
...
-- 旧数据在 M07 上线前已经通过既有发布链路可见；以创建时间保守回填。
-- 新文档在 M05 原子发布事务中写入这两个时间；发布前保持 NULL，天然不能通过 M07 回源复核。
UPDATE documents
...
-- M06 已创建但尚未执行的 Run 也必须冻结同一套 M07 参数，避免新代码解析旧快照失败。
UPDATE rag_runs
...
COMMENT ON COLUMN chunk_embedding_refs.vector_id IS
  'M07 批量 PG 回源键；Milvus 只返回该键，正文仍从本表关联 knowledge_chunks 读取。';
