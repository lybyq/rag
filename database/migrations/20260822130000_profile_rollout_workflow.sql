-- 索引构建与发布 IDX-016：Profile 重建必须经过候选构建、离线评测、灰度/全量发布和请求级回退。
...
-- 合成重建 Job 复用原文件执行 文件解析与OCR/知识加工与质量；此映射让 索引构建与发布 知道候选不能自动切换 Head。
...
-- 会话运行与事件/查询规划与混合检索 按稳定 userId Hash 选择候选；未实现在线检索前也能先完成安全发布事实建模。
...
COMMENT ON TABLE space_manifest_canaries IS
  'CANARY 候选不改稳定 Head；查询规划与混合检索 根据 userId 稳定哈希和 percent 路由，提升后才原子切 Head。';
