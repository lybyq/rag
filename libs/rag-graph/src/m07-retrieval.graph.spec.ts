...
import {
  createM07RetrievalGraph,
  M07RetrievalService,
  type M07RetrievalGraphInput,
  type M07RetrievalGraphDependencies,
} from './m07-retrieval.graph';
...
describe('[RET-009][RET-015] M07 LangGraph', () => {
  test('实际编译图执行 Dense/Sparse 并行召回、RRF、PG 回源与二轮停止', async () => {
    ...
    const graph = createM07RetrievalGraph(dependencies);
  ...
  test('Dense 故障时 Sparse 单路线安全降级并保留真实候选', async () => {
    ...
    const result = await createM07RetrievalGraph(dependencies).invoke(
  ...
  test('问候路线不调用模型、Embedding 或向量库', async () => {
    const source = sourceWithCandidates([]);
    const dependencies = graphDependencies(source);
    const result = await createM07RetrievalGraph(dependencies).invoke(initialState('你好'));
...
function debugService(source: RetrievalSourceRepository): M07RetrievalService {
  const authorization = {
    restrictRequestedSpaces: jest.fn(
      async (_context: unknown, spaces: readonly string[]) => spaces,
    ),
  } as unknown as AuthorizationService;
  return new M07RetrievalService(
...
function contextWithRole(role: 'KNOWLEDGE_READER' | 'SYSTEM_ADMIN'): AccessContext {
  return {
    user: createTestUserContext('m07-owner', [role]),
    requestId: 'm07-debug-request',
  };
}
...
function initialState(question: string): M07RetrievalGraphInput {
  return {
    runId: '77777777-7777-4777-8777-777777777777',
    context: {
      user: createTestUserContext('m07-user', ['SYSTEM_ADMIN']),
      requestId: 'm07-request',
    },
    question,
    historyEntities: [],
    manifests: [manifest],
