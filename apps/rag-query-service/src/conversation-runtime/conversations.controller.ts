/**
 * 会话运行与事件 会话、消息、Run 创建与反馈 HTTP Adapter。
...
import { z } from 'zod';
import { parseConversationInput, toAccessContext } from './conversation-http-utils';
...
export class ConversationsController {
  ...
  public async create(
    ...
    return this.envelope(
      await this.runs.createConversation(
        ...
        parseConversationInput(CreateConversationRequestSchema, rawBody),
      ),
    );
  }
  ...
  public async list(
    ...
    return this.envelope(
      await this.runs.listConversations(
        toAccessContext(user, this.requestContext),
        parseConversationInput(ListConversationsQuerySchema, rawQuery),
      ),
    );
  }
  ...
  > {
    return this.envelope(
      await this.runs.listMessages(
        toAccessContext(user, this.requestContext),
        parseConversationInput(IdSchema, rawConversationId),
      ),
    );
  }
  ...
  public async createRun(
    ...
    const key = parseConversationInput(z.string().min(8).max(200), idempotencyKey);
    return this.envelope(
      await this.runs.createRun(
        toAccessContext(user, this.requestContext),
        parseConversationInput(IdSchema, rawConversationId),
        key,
        parseConversationInput(CreateRagRunRequestSchema, rawBody),
      ),
    );
  }
...
export class MessageFeedbackController {
  ...
  public async save(
    ...
    const data = await this.runs.saveFeedback(
      toAccessContext(user, this.requestContext),
      parseConversationInput(IdSchema, rawMessageId),
      parseConversationInput(CreateMessageFeedbackRequestSchema, rawBody),
    );
    const traceId = this.requestContext.get()?.traceId;
    return {
      data,
