/** 会话运行与事件 Query API 的可信访问上下文、Zod 校验与 Cookie 工具。 */
 ...
 */
export function parseConversationInput<TSchema extends z.ZodType>(
  schema: TSchema,
  input: unknown,
): z.infer<TSchema> {
  const result = schema.safeParse(input);
  if (!result.success) throw new BadRequestException('请求参数不符合 会话运行与事件 接口契约');
  return result.data;
}
