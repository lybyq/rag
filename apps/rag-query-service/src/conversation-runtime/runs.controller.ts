/**
 * 会话运行与事件 Run 详情、取消、轮询、SSE 与一次性 Ticket HTTP Adapter。
...
import { z } from 'zod';
import { parseConversationInput, readCookie, toAccessContext } from './conversation-http-utils';
...
export class RunsController {
  ...
  public async get(
    ...
    return this.envelope(
      await this.runs.getRun(
        ...
        parseConversationInput(IdSchema, rawRunId),
      ),
    );
  }
  ...
  public async cancel(
    ...
    const body = parseConversationInput(CancelRagRunRequestSchema, rawBody);
    const run = await this.runs.cancelRun(
      toAccessContext(user, this.requestContext),
      parseConversationInput(IdSchema, rawRunId),
      body.reason,
    );
    this.metrics.conversationOperationsTotal.inc({ operation: 'cancel', result: 'success' });
  ...
  public async ticket(
    ...
    const runId = parseConversationInput(IdSchema, rawRunId);
  ...
  public async poll(
    ...
    const query = parseConversationInput(EventPollSchema, rawQuery);
  ...
  private async streamLoop(
    ...
    prepareSse(response);
    this.metrics.conversationSseConnections.inc({ transport });
    ...
    try {
      ...
      this.metrics.conversationSseConnections.dec({ transport });
...
export class RunTicketStreamController {
  ...
  public async stream(
    ...
    const ticket = parseConversationInput(TicketSchema, rawTicket);
    const redeemed = await this.runs.redeemStreamTicket(ticket, runId);
    prepareSse(response);
    this.metrics.conversationSseConnections.inc({ transport: 'ticket' });
    ...
    try {
      ...
      this.metrics.conversationSseConnections.dec({ transport: 'ticket' });
      if (!response.destroyed) response.end();
    }
  }
}
...
function parseLastEventId(value: string | undefined): number {
  return value ? parseConversationInput(z.coerce.number().int().nonnegative(), value) : 0;
}
